...
export class RegisterFormComponent implements OnInit {
	...
	constructor(
		...
		private authCommon: AuthModuleService,
	...
	ngOnInit() {
		...
		this.activatedRoute.queryParams.subscribe(params => {
			...
			if (this.code) {
				this.loginApi.confirmGuid(this.code).subscribe(
					(results: any) => {
	...
	onChangePassword() {
		let validate = this.authCommon.validatePassword(this.password);
		if (validate) {
	...
	onChangeEmail() {
		let validate = this.authCommon.validateEmail(this.emailAddress);
		if (validate) {
	...
	onpreSignUp() {
		if (!this.authCommon.validatePassword(this.password) && this.password) {
